...
export default function Register() {
  ...
  const registerGoogle = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      if (!companyName.trim()) {
        setError('Please enter a Company Name before using Google to register.');
        return;
      }
      setLoading(true);
      setError('');
      try {
        ...
        navigate('/dashboard'); // Auto-login on Google register success
      } catch (err: any) {
      ...
      <div className="flex-1 flex flex-col justify-center px-6 sm:px-12 lg:px-24 xl:px-32 relative py-12 overflow-y-auto">
        ...
        <div className="w-full max-w-sm mx-auto mt-8">
          ...
          <p className="text-[12px] text-slate-500 text-center mt-4">
            If registering with Google, please fill in the <strong>Company Name</strong> field above first.
          </p>

          <Button 
            type="button"
            variant="outline"
            className="w-full h-10 mt-3 border-slate-200 text-slate-700 hover:bg-slate-50 text-[14px] font-medium"
